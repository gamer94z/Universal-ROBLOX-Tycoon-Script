-- 0xVyrs universal tycoon scanner v3
-- Runtime owns scan timing. This module never returns a stale button snapshot
-- when it has been explicitly asked to scan an already-known root.
...
return function(context)
    ...
    local function candidateFromObject(o,depth)
        if not o or instanceBlocked(o) then return nil end
        local best=nil
        ...
        if a then
            for key,value in pairs(a) do
                if priceKey(key) then offer(value,125,key) end
            end
        end
    ...
    local function priceEvidence(interaction,root)
        ...
        while current and current~=workspace and depth<=4 do
            if blockedAround(current,root) then break end
            local nodes={current}
            local cursor=1
            local inspected=0
    ...
    local function entryName(host,interaction,evidence)
        if interaction:IsA("ProximityPrompt") then
            for _,v in ipairs({interaction.ObjectText,interaction.ActionText}) do
                local text=trim(v)
                if text~="" and not blockedText(text) and parseNumber(text)==nil then return text end
            end
        end
    ...
    local function discoverRoot()
        ...
        for _,interaction in ipairs(interactions) do
            local evidence=priceEvidence(interaction,nil)
            local blocked=blockedAround(interaction,nil)
            local part=interactionPart(interaction)
        ...
        for root,s in pairs(stats) do
            if s.interactions>0 then
                ...
                if score>=ROOT_MIN_SCORE then table.insert(candidates,{root=root,score=score,ownerVerified=s.owner,ownerSource=s.owner and (namedPlayerContainer(root) and "player-named-plot" or "owner-signal") or "structure",nearest=s.nearest,interactions=s.interactions,priced=usable}) end
            end
        end
    ...
    local function scanRoot(root,meta,mode)
        ...
        local data={
            root=root,rootName=root.Name,ownerMatch=explicit,ownerVerified=explicit,
        ...
        if allowed then
            ...
            for _,interaction in ipairs(descendants) do
                if isInteraction(interaction) and not blockedAround(interaction,root) then
                    local isCollector,collectorHost=collectorSemantic(interaction,root)
                    if isCollector then
                        local key=interaction
                        if not seenActivation[key] and #data.drops<(CONFIG.maxDrops or 60) then
                ...
                elseif isInteraction(interaction) then
        ...
        data.debug={scanMode=data.scanMode,scanTimeMs=data.scanTimeMs,snapshotSize=#descendants,rootScore=data.rootScore,ownerSource=data.ownerSource,paidSkipped=data.paidSkipped}
        return data
    end

    local function emptyData(elapsed,size)
        return {root=workspace,rootName="Workspace",ownerMatch=false,ownerVerified=false,ownerSource="none",selectedByPosition=false,automationAllowed=CONFIG.requireOwnerMatch==false,safeAutomation=CONFIG.requireOwnerMatch==false,owned=false,cash=tonumber(context.getCash()),buttons={},drops={},affordableCount=0,lockedCount=0,totalButtons=0,progressPercent=0,scanMode="structural-discovery",scanTimeMs=elapsed,debug={scanMode="structural-discovery",scanTimeMs=elapsed,reason="no-root",worldSnapshotSize=size or 0}}
    end

    local function scan()
        if knownRoot and knownRoot.Parent then
            return scanRoot(knownRoot,knownMeta or {},"known-root")
        end
        local now=os.clock()
    ...
    return {scan=scan,invalidateRoot=invalidateRoot,parseCompactNumber=parseNumber}
end
